...
const menu_attrs_post_schema = Joi.object({
    ...
    promotion: Joi.number().positive().max(1).required()
        .messages({
...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            const products = await Promise.all(raw.products.map((id) => models.product.model.findById(id)));

            const menu = await models.menu.model.create({
                name: raw.name.trim().replace(/  +/g, ''),
                products: products.reduce<(IProduct & db.Document)[]>((acc, product) => {
                    if (product !== null) {
                        acc.push(product);
                    }
...
router.put(
    ...
    async (req, res) => {
        try {
            ...
            const body = req.body as IMenuSelfPut

            const menu = await models.menu.model.findByIdAndUpdate(id, body)
